    ...
    int maximumAmount(vector<vector<int>>& coins) {
        ...
        while(q.size()){
            ...
            for(int i=0; i<2; i++){
                ...
                bool chk = false;
                if(coins[nr][nc] >= 0){
                    for(int t=2; t>-1; t--) if(dp[nr][nc][t] < dp[now.first][now.second][t] + coins[nr][nc]){
                        dp[nr][nc][t] = dp[now.first][now.second][t] + coins[nr][nc];
                        chk = true;
                    }
                }
                else{
                    // 막기 씀
                    for(int t=2; t>0; t--) if(dp[nr][nc][t - 1] < dp[now.first][now.second][t]){
                        dp[nr][nc][t - 1] = dp[now.first][now.second][t];
                        chk = true;
                    }
                    // 막기 안씀
                    for(int t=2; t>-1; t--) if(dp[nr][nc][t] < dp[now.first][now.second][t] + coins[nr][nc]){
